...
public class RMF_RadialMenu : MonoBehaviour {
   ...
   [HideInInspector]
    ...
    void Awake() {
		...
		GameObject lmgr = GameObject.FindGameObjectWithTag ("LeactMgr");

		leactmgr = lmgr.GetComponent<LeactManager> ();
		handListener = leactmgr.kinectmgr.GetComponent<SimpleHandListener>();
    ...
    void Update() {
		...
		//SELF IMPLEMENTED KINECT ANGLE//
		currentAngle = normalizeAngle(kinectAngle - globalOffset + (angleOffset / 2f));
...
//                selectionFollowerContainer.rotation = Quaternion.Euler(0, 0, rawAngle + 270);
	...
	public void setKinectAngle(float angle){
		kinectAngle = angle;
	}
	...
	public void setItemText(int itemNr, string text){


		Text elemText = elements [itemNr-1].GetComponentInChildren<Text> ();
		elemText.text = text;
	
//		elements [itemNr].label = text;
	}
